...
- **Zero Agents** - the `zero` CLI: an interactive TUI (`zero`) and a
  headless mode (`zero -p`) for scripts/CI, both over the same engine, with
  write tools, git checkpointing, and a model gateway.
