...
test("treats a leading-dot file as having no extension, even if it looks like a known one", () => {
  expect(mimeTypeFor(".png")).toBe("application/octet-stream");
  expect(mimeTypeFor("dir/.gitignore")).toBe("application/octet-stream");
});
