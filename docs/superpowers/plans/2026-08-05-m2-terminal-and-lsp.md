...
## Task 3: Daemon — `PtyService` (Node-worker bridge)

> **Revised during execution.** The original version of this task spawned
> `node-pty` directly inside the Bun daemon process. That does not work:
> confirmed live, `node-pty`'s native binding under Bun 1.3.14 emits exactly
> one `onData` event and then the PTY stream goes permanently silent — no
> shell prompt, no command output, ever — while the identical code and
> `node_modules` tree behaves correctly under plain Node (full interactive
> shell, real command output). This is a known upstream gap:
> [oven-sh/bun#7362](https://github.com/oven-sh/bun/issues/7362), "node-pty
> unable to be run from bun." `PtyService` therefore hosts `node-pty` in a
> separate, plain-JavaScript child process spawned under real `node` (never
> `bun`), and talks to it over newline-delimited JSON on stdio. Everything
> downstream of `PtyService` — Task 4's RPC wiring, Task 9's `PtyStore`,
> Tasks 10-11's terminal UI — is completely unaffected: `PtyService`'s
> public API (`open`/`input`/`resize`/`close`/`list`/`closeAll`, all
> synchronous, same signatures) is unchanged. Only this task's internals and
> test differ from the original version.

**Files:**
- Create: `packages/daemon/src/pty-worker.js` (plain JS/ESM, executed via `node`, never bundled/transpiled — hosts `node-pty` directly)
...
- Produces: `PtyService` class with `open(shell: string | undefined, cols: number, rows: number): { sessionId: string; shell: string }`, `input(sessionId: string, data: string): void`, `resize(sessionId: string, cols: number, rows: number): void`, `close(sessionId: string): void`, `list(): PtySessionInfo[]`, `closeAll(): void`. Constructor: `new PtyService(cwd: string, onOutput: (sessionId: string, data: string) => void, onExit: (sessionId: string, exitCode: number) => void)`. Identical to the original design — callers (Task 4) need no changes.

Wire protocol between the Bun-side `PtyService` and the Node-side worker, one JSON object per line on stdio:
- Parent → worker: `{ type: "open", sessionId, shell, cols, rows, cwd }`, `{ type: "input", sessionId, data }`, `{ type: "resize", sessionId, cols, rows }`, `{ type: "close", sessionId }`, `{ type: "closeAll" }`.
- Worker → parent: `{ event: "data", sessionId, data }`, `{ event: "exit", sessionId, exitCode }`.
- `sessionId` is generated by the **parent** (`randomUUID()`), not the worker — so `open()` can stay synchronous on the Bun side (it returns immediately; the worker spawns the real PTY asynchronously and starts streaming `data` events once ready). The worker never needs to ack `open`.
- The worker guards its own `onData`/`onExit` against a `sessionId` already removed from its local map (by an explicit `close`), so no event arrives for a session the parent has already forgotten — the same double-fire hazard as the original design, resolved the same way, just on the worker side.
...
Create `packages/daemon/src/pty-worker.js`. Plain JavaScript, not TypeScript — this file is executed directly by `node`, never by Bun or through any build step, so it must need no transpilation. It uses ESM `import` syntax because `packages/daemon/package.json` declares `"type": "module"`, which Node also honors for plain `.js` files in this package:
...
Create `packages/daemon/src/pty.test.ts` — same observable behavior as the original design (these tests exercise `PtyService`'s public API only, so they're agnostic to the worker-process internals):
...
Note the deliberate difference from the original design's `closeAll` test: that version asserted `onExit` fires synchronously for every session on `closeAll()`, driving the original single-process implementation's synchronous-fire trick. In the worker-bridge design, `onExit` events for an explicit `close`/`closeAll` are **not** synthesized — the parent already knows those sessions are gone (it just told the worker to kill them and cleared its own map), so a synchronous `list()` check after `close`/`closeAll` is the right assertion, not an `exits` callback. (A `pty/exit` broadcast on close, useful for notifying *other* browser tabs, is Task 4's concern via `PtyService`'s constructor callback firing for natural process exits — this task's job is just the map bookkeeping and the worker bridge.)
...
Run: `bun test packages/daemon/src/pty.test.ts`
Expected: PASS. This spawns a real `node` subprocess per `PtyService` instance — each test creates its own instance and each must call `service.closeAll()` (which also kills the worker process) so tests don't leak `node` processes; check this after a full run with `ps aux | grep pty-worker` showing nothing left over.

- [ ] **Step 7: Typecheck and commit**

Run: `bunx tsc -b`
Expected: no errors. (`pty-worker.js` is plain JS and outside the TS project graph — it is not type-checked, which is expected and fine.)

```bash
git add packages/daemon/package.json packages/daemon/src/pty.ts packages/daemon/src/pty.test.ts packages/daemon/src/pty-worker.js bun.lock
git commit -m "feat(daemon): add PtyService (node-pty hosted in a Node worker, node-pty is broken under Bun — oven-sh/bun#7362)"
```
